...
abstract class BaseFilmTypesPeer {
	...
	public static function getMapBuilder()
	{
		if (self::$mapBuilder === null) {
			self::$mapBuilder = new FilmTypesMapBuilder();
		}
    ...
    foreach (sfMixer::getCallables('BaseFilmTypesPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseFilmTypesPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseFilmTypesPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseFilmTypesPeer', $criteria, $con);
    }
  ...
  static public function getUniqueColumnNames()
  {
	...
	{
		return FilmTypesPeer::CLASS_DEFAULT;
	}
    ...
    foreach (sfMixer::getCallables('BaseFilmTypesPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseFilmTypesPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseFilmTypesPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseFilmTypesPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseFilmTypesPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseFilmTypesPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseFilmTypesPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseFilmTypesPeer', $values, $con, $ret);
    }
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		foreach ($objects as $obj) {
			...
			$c->add(FilmFilmTypesPeer::FILM_GENRE_ID, $obj->getId());
	...
	public static function doValidate(FilmTypes $obj, $cols = null)
	{
		...
		$res =  BasePeer::doValidate(FilmTypesPeer::DATABASE_NAME, FilmTypesPeer::TABLE_NAME, $columns);
    if ($res !== true) {
        $request = sfContext::getInstance()->getRequest();
        foreach ($res as $failed) {
            $col = FilmTypesPeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
        }
    }
...
//
// NOTE: This static code cannot call methods on the FilmTypesPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the FilmTypesPeer class:
//
// Propel::getDatabaseMap(FilmTypesPeer::DATABASE_NAME)->addTableBuilder(FilmTypesPeer::TABLE_NAME, FilmTypesPeer::getMapBuilder());
//
// Doing so will effectively overwrite the registration below.

Propel::getDatabaseMap(BaseFilmTypesPeer::DATABASE_NAME)->addTableBuilder(BaseFilmTypesPeer::TABLE_NAME, BaseFilmTypesPeer::getMapBuilder());
